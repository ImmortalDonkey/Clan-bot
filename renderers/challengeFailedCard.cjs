...
async function createChallengeFailedCard(options) {
  const {
    challengeId,
    clanName,
    pokemons,
  ...
  const rightMaxHeight = CARD_HEIGHT - MARGIN * 2;
  ...
  const infoBoxHeight = leftHeight * 0.70;
  const bottomBoxHeight = leftHeight - infoBoxHeight - 40;
  ...
  const lines = [];
  lines.push({ label: "Clan:", value: clanName || "Clan Challenge" });
  lines.push({ spacer: true });

  lines.push({ label: "Target:", value: pokemonList[0] });
  for (let i = 1; i < pokemonList.length; i++) {
    lines.push({ label: "", value: pokemonList[i] });
  }

  lines.push({ label: "Rarity:", value: rarityLabel || "Unknown" });
  lines.push({ spacer: true });

  lines.push({ label: "Points:", value: pointsLabel });
  ...
  let cy = infoBoxY + (infoBoxHeight - lines.length * LINE_GAP) / 2 + LINE_GAP;
  ...
  fs.writeFileSync(
    path.join(CARDS_DIR, `challengeEnd_${challengeId}_failed.png`),
    buffer
  );
...
module.exports = { createChallengeFailedCard };
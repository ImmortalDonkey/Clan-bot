...
const { getHighestRarityForList, getRarityDisplayLabel, pointsForRarity } = require('./rarity.cjs');
...
async function postChallengeCard(client, challenge) {
  ...
  if (!channelId) throw new Error('CHALLENGE_CHANNEL_ID missing');

  const channel = await client.channels.fetch(channelId).catch(() => null);
  if (!channel) throw new Error('Challenge channel not found');
...
function startChallengeScheduler(client) {
  ...
  // First tick quickly, then every 60s
  tick().catch(console.error);
  setInterval(() => tick().catch(console.error), 60_000);
}

module.exports = { startChallengeScheduler, postChallengeCard };